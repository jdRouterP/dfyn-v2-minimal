...
library Ticks {
    ...
    {
        uint256 price = (sqrtpriceX96 * 10**6) / (2**96);
        price = (price**2);
        uint256 desiredTokenOut;
        uint256 limitOrderLiquidity;
        if (zeroForOne) {
            desiredTokenOut = (input * price) / 10**12;
            limitOrderLiquidity = limitOrderTicks[tick].token1Liquidity;
            //Checks whether tick has liquidity
            if (limitOrderLiquidity > 0) {
                //check if limitOrderliquidity can fullfill
                if (limitOrderLiquidity >= desiredTokenOut) {
                    limitOrderTicks[tick].token1Liquidity -= desiredTokenOut;
                    limitOrderTicks[tick].token0Claimable += input;
                    amountOut += desiredTokenOut;
                    input = 0;
                    cross = false;
                } else {
                    input -= ((limitOrderLiquidity * 10**12) / price);
                    limitOrderTicks[tick].token1Liquidity = 0;
                    limitOrderTicks[tick].token0Claimable += input;
                    amountOut += limitOrderLiquidity;
                }
            }
        } else {
            desiredTokenOut = (input * 10**12) / price;
            limitOrderLiquidity = limitOrderTicks[tick].token0Liquidity;
            //Checks whether tick has liquidity
            if (limitOrderLiquidity > 0) {
                //check if limitOrderliquidity can fullfill completely
                if (limitOrderLiquidity >= desiredTokenOut) {
                    limitOrderTicks[tick].token0Liquidity -= desiredTokenOut;
                    limitOrderTicks[tick].token1Claimable += input;
                    amountOut += desiredTokenOut;
                    input = 0;
                    cross = false;
                } else {
                    //partial fullfill
                    input -= ((limitOrderLiquidity * price) / 10**12);
                    limitOrderTicks[tick].token0Liquidity = 0;
                    limitOrderTicks[tick].token1Claimable += ((limitOrderLiquidity * price) / 10**12);
                    amountOut += limitOrderLiquidity;
                }
            }
        }
        return (input, amountOut, cross);
    }
}
