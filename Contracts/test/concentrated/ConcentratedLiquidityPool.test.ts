...
describe("Concentrated Liquidity Product Pool", function () {
  ...
  describe("Valid actions", async () => {
    it("Should mint liquidity (in / out of range, native / from vault, reusing ticks / new ticks)", async () => {
    ...
    it("should increase liquidity", async () => {
      for (const pool of trident.concentratedPools) {
        ...
        addLiquidityParams = {
          pool: pool,
          amount0Desired: getBigNumber(10),
          amount1Desired: getBigNumber(10),
          native: false,
          lowerOld: helper.insert(lower),
          lower,
          upperOld: helper.insert(upper),
    ...
    it("Should add liquidity and swap (without crossing)", async () => {
      for (const pool of trident.concentratedPools) {
        ...
        expect((await pool.getImmutables())._swapFee.toString()).to.be.equals("1");

        // update swapFee
        await trident.concentratedPoolFactory.updateSwapFee(pool.address, 5);
        ...
        expect((await pool.getImmutables())._swapFee.toString()).to.be.equals("5");
      }
    });
    ...
    it("Should collect protocolFee", async () => {
      for (const pool of trident.concentratedPools) {
        ...
        const oldBarFeeToBalanceToken0 = await trident.vault.balanceOf(token0, barFeeTo);
        const tickAtPrice = await getTickAtCurrentPrice(pool);
        const nearestValidTick = tickAtPrice - (tickAtPrice % tickSpacing);
        const nearestEvenValidTick =
          (nearestValidTick / tickSpacing) % 2 == 0 ? nearestValidTick : nearestValidTick + tickSpacing;
        const oldBarFeeToBalanceToken1 = await trident.vault.balanceOf(token1, barFeeTo);
        ...
        const barFeeToBalanceToken0 = await trident.vault.balanceOf(token0, barFeeTo);
        const barFeeToBalanceToken1 = await trident.vault.balanceOf(token1, barFeeTo);
    ...
    it("Should create incentive", async () => {
      ...
      const rewardsA = await trident.vault.balanceOf(trident.extraToken.address, recipientA);
      const rewardsB = await trident.vault.balanceOf(trident.extraToken.address, recipientB);
      ...
      const newRewardsA = await trident.vault.balanceOf(trident.extraToken.address, recipientA);
      const newRewardsB = await trident.vault.balanceOf(trident.extraToken.address, recipientB);
      const newRewardsC = await trident.vault.balanceOf(trident.extraToken.address, recipientC);
