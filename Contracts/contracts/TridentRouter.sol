...
contract TridentRouter is ITridentRouter, SelfPermit, Multicall {
    ...
    /// @notice Swaps token A to token B directly. Swaps are done on `vault` tokens.
    /// @param params This includes the address of token A, pool, amount of token A to swap,
    /// minimum amount of token B after the swap and data required by the pool for the swap.
    /// @dev Ensure that the pool is trusted before calling this function. The pool can steal users' tokens.
    function exactInputSingle(ExactInputSingleParams calldata params) public payable returns (uint256 amountOut) {
        // Prefund the pool with token A.
        vault.transfer(params.tokenIn, msg.sender, params.pool, params.amountIn);
    ...
    function exactInput(ExactInputParams calldata params) public payable returns (uint256 amountOut) {
        ...
        vault.transfer(params.tokenIn, msg.sender, params.path[0].pool, params.amountIn);
    ...
    /// it takes raw ERC-20 tokens from the users and deposits them into `vault`.
    /// @param params This includes the address of token A, pool, amount of token A to swap,
    /// minimum amount of token B after the swap and data required by the pool for the swap.
    /// @dev Ensure that the pool is trusted before calling this function. The pool can steal users' tokens.
    function exactInputSingleWithNativeToken(ExactInputSingleParams calldata params) public payable returns (uint256 amountOut) {
        // Deposits the native ERC-20 token from the user into the pool's `vault`.
        _depositToVault(params.tokenIn, params.pool, params.amountIn);
        // Trigger the swap in the pool.
        amountOut = IPool(params.pool).swap(params.data);
        // Ensure that the slippage wasn't too much. This assumes that the pool is honest.
        if (amountOut < params.amountOutMinimum) revert TooLittleReceived();
    ...
    function exactInputWithNativeToken(ExactInputParams calldata params) public payable returns (uint256 amountOut) {
        // Deposits the native ERC-20 token from the user into the pool's `vault`.
        _depositToVault(params.tokenIn, params.path[0].pool, params.amountIn);
        // Call every pool in the path.
        // Pool `N` should transfer its output tokens to pool `N+1` directly.
        // The last pool should transfer its output tokens to the user.
        for (uint256 i; i < params.path.length; ) {
    ...
    function complexPath(ComplexPathParams calldata params) public payable {
        ...
        for (uint256 i; i < params.initialPath.length; ) {
            if (params.initialPath[i].native) {
                _depositToVault(params.initialPath[i].tokenIn, params.initialPath[i].pool, params.initialPath[i].amount);
            } else {
                vault.transfer(params.initialPath[i].tokenIn, msg.sender, params.initialPath[i].pool, params.initialPath[i].amount);
            }
        ...
        for (uint256 i; i < params.percentagePath.length; ) {
            uint256 balanceShares = vault.balanceOf(params.percentagePath[i].tokenIn, address(this));
            uint256 transferShares = (balanceShares * params.percentagePath[i].balancePercentage) / uint256(10)**8;
            vault.transfer(params.percentagePath[i].tokenIn, address(this), params.percentagePath[i].pool, transferShares);
        ...
        for (uint256 i; i < params.output.length; ) {
            uint256 balanceShares = vault.balanceOf(params.output[i].token, address(this));
            if (balanceShares < params.output[i].minAmount) revert TooLittleReceived();
            if (params.output[i].unwrapVault) {
                vault.withdraw(params.output[i].token, address(this), params.output[i].to, 0, balanceShares);
            } else {
                vault.transfer(params.output[i].token, address(this), params.output[i].to, balanceShares);
            }
            unchecked {
                ++i;
            }
    ...
    function addLiquidity(
        ...
        for (uint256 i; i < tokenInput.length; ) {
            if (tokenInput[i].native) {
                _depositToVault(tokenInput[i].token, pool, tokenInput[i].amount);
            } else {
                vault.transfer(tokenInput[i].token, msg.sender, pool, tokenInput[i].amount);
            }
            unchecked {
                ++i;
            }
    ...
    /// @notice Burn liquidity tokens to get back `vault` tokens.
    /// @param pool Pool address.
    /// @param liquidity Amount of liquidity tokens to burn.
    /// @param data Data required by the pool to burn liquidity.
    /// @param minWithdrawals Minimum amount of `vault` tokens to be returned.
    ...
    function sweep(
        ...
        if (onVault) {
            vault.transfer(token, address(this), recipient, amount);
        } else {
    ...
    /// @notice Wrapper function to allow vault set master contract approval to be batched, so the first trade can happen in one transaction.
    function approveMasterContract(
        uint8 v,
        bytes32 r,
        bytes32 s
    ) external payable {
        vault.setMasterContractApproval(msg.sender, address(this), true, v, r, s);
    }

    /// @notice Deposit from the user's wallet into Vault.
    /// @dev Amount is the native token amount. We let Vault do the conversion into shares.
    function _depositToVault(
        address token,
        address recipient,
        uint256 amount
    ) internal {
        vault.deposit{value: token == USE_NATIVE ? amount : 0}(token, msg.sender, recipient, amount, 0);
    }
