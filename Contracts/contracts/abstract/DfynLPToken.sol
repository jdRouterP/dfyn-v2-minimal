...
abstract contract DfynLPToken is ERC721 {
    ...
    constructor() ERC721("DFYN_LP", "DLP") {
    ...
    function permit(
        ...
        unchecked {
            ...
            require(recoveredAddress == tokenOwner || isApprovedForAll(tokenOwner, recoveredAddress), "INVALID_SIGNER");
        }
        ...
        unchecked {
            bytes32 digest = keccak256(
                abi.encodePacked(
                    "\x19\x01",
                    DOMAIN_SEPARATOR(),
                    keccak256(abi.encode(PERMIT_ALL_TYPEHASH, tokenOwner, operator, noncesForAll[tokenOwner]++, deadline))
                )
            );
            address recoveredAddress = ecrecover(digest, v, r, s);
            require(
                (recoveredAddress != address(0) && recoveredAddress == tokenOwner) || isApprovedForAll(tokenOwner, recoveredAddress),
                "INVALID_PERMIT_SIGNATURE"
            );
        }
        _setApprovalForAll(tokenOwner, operator, true);
    }
}
