...
contract ConcentratedLiquidityPoolStaker {
    ...
    constructor(IPoolManager _poolManager) {
        ...
        IVaultMinimal _vault = IVaultMinimal(_poolManager.vault());
    ...
    function reclaimIncentive(
        ...
        _transfer(incentive.token, address(this), receiver, amount, unwrapVault);
    ...
    function claimRewards(
        ...
        for (uint256 i = 0; i < incentiveIds.length; i++) {
            ...
            _transfer(incentive.token, address(this), recipient, rewards, unwrapVault);
    ...
    function _transfer(
        ...
        if (unwrapVault) {
            vault.withdraw(token, from, to, 0, shares);
        } else {
            vault.transfer(token, from, to, shares);
        }
    }
}
