...
contract TridentSushiRollCP is SelfPermit, Multicall {
    ...
    function migrate(
        ...
        (uint256 amount0, uint256 amount1) = pair.burn(address(vault));

        if (amount0 < minToken0Received || amount1 < minToken1Received) revert MinimumOutput();

        vault.deposit(token0, address(vault), tridentPool, amount0, 0);
        vault.deposit(token1, address(vault), tridentPool, amount1, 0);
