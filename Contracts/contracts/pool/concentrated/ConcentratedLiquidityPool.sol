...
/// @dev Amounts are considered to be in Vault shared
contract ConcentratedLiquidityPool is IConcentratedLiquidityPoolStruct {
    ...
    uint24 internal swapFee; /// @dev 1000 corresponds to 0.1% fee. Fee is measured in pips.
    uint128 internal immutable MAX_TICK_LIQUIDITY;

    address internal immutable barFeeTo;
    IVaultMinimal internal immutable vault;
    IMasterDeployer internal immutable masterDeployer;

    address internal immutable token0;
    address internal immutable token1;

    /// @dev Owner of the pool, i.e. factory
    ...
    uint128 internal reserve0; /// @dev `vault` share balance tracker.
    ...
    constructor(bytes memory _deployData, IMasterDeployer _masterDeployer) {
        ...
        vault = IVaultMinimal(_masterDeployer.vault());
    ...
    /// @dev Function to update the swap fee. Called only from the factory.
    ...
    function createLimitOrder(
        ...
        require(amountIn > 0, "Amount cant be zero");
        if (zeroForOne) {
            IPositionManager(msg.sender).limitOrderCallback(token0, amountIn, native);
            limitOrderTicks[tick].token0Liquidity += amountIn;
        } else {
            IPositionManager(msg.sender).limitOrderCallback(token1, amountIn, native);
            limitOrderTicks[tick].token1Liquidity += amountIn;
        }
    }
    ...
    function claimLimitOrder(
        ...
        if (zeroForOne) {
            require(limitOrderTicks[tick].token1Claimable >= amount);
            limitOrderTicks[tick].token1Claimable -= amount;
            _transferToken(token1, msg.sender, amount);
        } else {
            require(limitOrderTicks[tick].token0Claimable >= amount);
    ...
    function cancelLimitOrder(
        ...
        if (zeroForOne) {
            require(limitOrderTicks[tick].token0Liquidity >= amount, "Fund Liquidated");
            limitOrderTicks[tick].token0Liquidity -= amount;
            _transferToken(token0, msg.sender, amount);
        } else {
            require(limitOrderTicks[tick].token1Liquidity >= amount, "Fund Liquidated");
            limitOrderTicks[tick].token1Liquidity -= amount;
            _transferToken(token1, msg.sender, amount);
        }
    }
    ...
    function swap(bytes memory data) public lock returns (uint256 amountOut) {
        (bool zeroForOne, address recipient, bool unwrapVault) = abi.decode(data, (bool, address, bool));
        ...
        while (cache.input != 0) {
            ...
            if (cross) {
                (cache.input, amountOut, cross) = Ticks.excecuteLimitOrder(
                    limitOrderTicks,
                    nextTickPrice,
            ...
            if (cross) {
                ...
                if (cache.currentLiquidity == 0) {
                    ...
                    //TODO:ADD limit order excecution here as well;
                    (cache.input, amountOut, cross) = Ticks.excecuteLimitOrder(
                        limitOrderTicks,
                        cache.currentPrice,
                        cache.nextTickToCross,
    ...
    function _balance(address token) internal view returns (uint256 balance) {
        balance = vault.balanceOf(token, address(this));
    }
    ...
    function _transferBothTokens(
        ...
        vault.transfer(token0, address(this), to, shares0);
        vault.transfer(token1, address(this), to, shares1);
    }
