...
contract Migrator {
    ...
    function migrate(IUniswapV2Minimal oldPool) external returns (address) {
        ...
        // Remove the liquidity and send assets to Vault.
        oldPool.transferFrom(address(masterChef), address(oldPool), lpBalance);
        (uint256 amount0, uint256 amount1) = oldPool.burn(address(vault));

        vault.deposit(token0, address(vault), address(pool), amount0, 0);
        vault.deposit(token1, address(vault), address(pool), amount1, 0);
