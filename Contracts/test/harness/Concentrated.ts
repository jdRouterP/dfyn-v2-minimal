...
export async function collectFees(params: {
  ...
  const { pool, tokenId, recipient, unwrapVault } = params;
  ...
  await Trident.Instance.concentratedPoolManager.collect(tokenId, recipient, unwrapVault);
...
export async function swapViaRouter(params: {
  ...
  const { pool, zeroForOne, inAmount, recipient, unwrapVault } = params;
  const immutables = await pool.getImmutables();
  ...
  const swapData = getSwapData({ zeroForOne, inAmount, recipient, unwrapVault });
  const routerData = {
    amountIn: inAmount,
...
export async function removeLiquidityViaManager(params: {
  ...
  const { pool, tokenId, liquidityAmount, recipient, unwrapVault } = params;
  ...
  const oldUserBalances = await Trident.Instance.getTokenBalance(tokens, recipient, unwrapVault);
  const oldPoolBalances = await Trident.Instance.getTokenBalance(tokens, pool.address, false);
  ...
  await manager.burn(tokenId, liquidityAmount, recipient, unwrapVault, 0, 0);
  ...
  const newUserBalances = await Trident.Instance.getTokenBalance(tokens, recipient, unwrapVault);
  const newPoolBalances = await Trident.Instance.getTokenBalance(tokens, pool.address, false);
...
export function getSwapData(params: {
  ...
  const { zeroForOne, recipient, unwrapVault } = params;
  return ethers.utils.defaultAbiCoder.encode(["bool", "address", "bool"], [zeroForOne, recipient, unwrapVault]);
}
